...
if [ $(grep -c "data" fqrft.txt) -eq 1 ]
then n=$(cat fqrft.txt | grep -o "num.*" | cut -d ',' -f1 | cut -d '"' -f3)
if [ ! -n "$n" ]
then n=0
fi
...
echo $dt $un$zlg今日已阅读$n次，今日总奖励$s币，本小时剩余未阅读$(cat rest.txt)次，当前$(cat y.txt)可提现 $msg > rft.txt
